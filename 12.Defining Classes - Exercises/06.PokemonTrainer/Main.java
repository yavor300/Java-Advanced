...
//    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (!"Tournament".equals(input)) {
            ...
            if (trainersNames.add(trainerName)) {
        ...
        String element = bfr.readLine();
        while (!"End".equals(element)) {
            for (Trainer trainer : trainers) {
                List<Pokemon> pokemons = trainer.getPokemons();
                boolean isPokemonWithElementExisting = false;

                for (Pokemon pokemon : pokemons) {
                    if (pokemon.getElement().equals(element)) {
                        isPokemonWithElementExisting = true;
                        trainer.setBadges(trainer.getBadges() + 1);
                        break;
                    }
                }

                if (!isPokemonWithElementExisting) {
            ...
            element = bfr.readLine();
        }

        trainers.stream().sorted((f, s) -> Integer.compare(s.getBadges(), f.getBadges()))
