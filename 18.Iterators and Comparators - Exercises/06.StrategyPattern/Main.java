...
//    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        Set<Person> peopleWithNameComp = new TreeSet<>(new ComparePeopleByName());
        Set<Person> peopleWithAgeComp = new TreeSet<>(new ComparePeopleByAge());

        int n = Integer.parseInt(bfr.readLine());
        for (int i = 0; i < n; i++) {
            String[] tokens = bfr.readLine().split("\\s+");
            Person person = new Person(tokens[0], Integer.parseInt(tokens[1]));
            peopleWithNameComp.add(person);
            peopleWithAgeComp.add(person);
        }

        for (Person person : peopleWithNameComp) {
            System.out.println(person.getName() + " " + person.getAge());
        }
        for (Person person : peopleWithAgeComp) {
            System.out.println(person.getName() + " " + person.getAge());
        }
